...
data = createDummyFeatures(as.data.frame(data), target = "TARGET",
                           ...
                           method = "reference")
# ##########################################################################################
# fwrite(data,"data.csv") # 此处save data
...
index = sample(c(1,0), nrow(train), replace = TRUE, prob = c(0.8,0.2))
...
write.csv(prediction, file.path(root,"predict", "submisssions2307_4.csv"), row.names = FALSE)
